...
System_Boundary(vorbereiter, "HTW Prüfungsvorbereiter") {
    ...
    ContainerDb(db, "Datenbank", "", "speichert Karteikarten, Fragen, Forumeintäge, etc.")
    Container(api, "API App", "", "stellt Funktionalitäten zum Abfragen von Daten via API bereit")
}
...
Rel_Back_Neighbor(db, api, "liest von und schreibt nach", "")
....
